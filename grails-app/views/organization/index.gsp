		...
		<div id="list-organization" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<th><g:message code="organization.parent.label" default="Parent" /></th>
					
						<g:sortableColumn property="text" title="${message(code: 'organization.text.label', default: 'Text')}" />
					
						<g:sortableColumn property="pxh" title="${message(code: 'organization.pxh.label', default: 'Pxh')}" />
				...
				<g:each in="${organizationInstanceList}" status="i" var="organizationInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: organizationInstance, field: "parent")}</td>
					
						<td>${fieldValue(bean: organizationInstance, field: "text")}</td>
					
						<td>${fieldValue(bean: organizationInstance, field: "pxh")}</td>
